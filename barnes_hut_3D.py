...
# or an abstract node of the quad-tree if it has child.
    ...
    def __init__(self, m, x, y, z):#Alejandro
    # The initializer creates a child-less node (an actual body).
        ...
        self.m_pos = m * array([x, y, z])#Alejandro
        self.momentum = array([0., 0., 0.])#Alejandro
        self.child = None
    ...
    # Places node into next-level quadrant and returns the quadrant number.
        ...
        return self._subdivide(2) + 2*self._subdivide(1) + 4*self._subdivide(0) #Alejandro
...
# a new body into a quad-tree node. Returns an updated version of the node.
    ...
    if node is not None and node.s > smallest_quadrant:
        ...
        #    ... Subdivide the region further by creating four children
            new_node.child = [None for i in range(8)]#Alejandro
...
# ffmpeg -r 15 -i bodies_%06d.png -q:v 0 bodies.avi
    ...
    ax.set_ylim([0., 1.0])
    ax.set_zlim([0., 1.0])    
    plt.gcf().savefig('imagenes/bodies3D_{0:06}.png'.format(i))
...
for i in range(max_iter):
    ...
    if i == 500:
        print('bodies[0]: ' + str(bodies[0].m_pos[2]/bodies[0].m))
